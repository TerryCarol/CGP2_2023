...
void SystemClass::Run()
{
	...
	while(!done)
	{
		...
		syncSpeed = m_Timer->GetSpeed();
		m_Graphics->SetSyncSpeed(syncSpeed);

		//마우스로 카메라 회전
		m_Input->GetCameraRotationInput(yaw, pitch);
		m_Graphics->CameraRotation(yaw, pitch);

		//키보드로 카메라 이동
		if (m_Input->IsWPressed()) {
			m_Graphics->CameraMovement('W');
		}
		if (m_Input->IsSPressed()) {
			m_Graphics->CameraMovement('S');
		}
		if (m_Input->IsAPressed()) {
			m_Graphics->CameraMovement('A');
		}
		if (m_Input->IsDPressed()) {
			m_Graphics->CameraMovement('D');
		}
